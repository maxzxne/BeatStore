...
export const guideSections = [
    ...
    blocks: [
      ...
      },
      {
        ...
        items: [
          ...
          'Сайт — hero, баннеры, прайс услуг, промо (акции + промокоды), футер, настройки',
    ...
    blocks: [
      {
        ...
        items: [
          ...
          'Баннеры / Промо — проверить даты и статусы активных акций.',
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'tip',
        text: 'Промокоды привязаны к username — сначала найдите пользователя здесь, потом создайте код в «Промо → Промокоды».',
      },
    ],
  },
  {
    ...
    blocks: [
      {
        type: 'p',
        text: 'Операционка service-заказов с витрины /order (и /order/ads, если реклама включена). Сроки и цены — в «Сайт → Прайс услуг» (вкладки Цены / Тексты). На /order в шаге «Я знаю, что хочу» клиент видит блок «Как считается цена» (таблица кликабельна). ТЗ в подробной форме обязательно; срок по умолчанию — 2–3 недели (21 день), если такой слот есть в прайсе.',
      },
      {
        type: 'h3',
  ...
  },
  {
    ...
    blocks: [
      ...
      {
        type: 'tip',
        text: 'Ставку ₽/день и очередь заявок правите здесь (вкладка «Заявки»). Скидку на рекламу — в «Промо → Акции» (scope ads).',
      },
    ],
  },
  {
    ...
    publicPaths: ['/order'],
    keywords: ['прайс', 'срок', 'предоплата', 'трэп', 'услуг', 'цены'],
    blocks: [
      {
        type: 'p',
        text: 'Отдельный раздел: сроки и суммы для формы «Я знаю, что хочу». Не путать со ставкой рекламы (Баннеры → Заявки). На /order в шагах «Услуги» и «Итог» клиент открывает прайс через значок «?» в кружке.',
      },
      {
        type: 'ul',
        items: [
          'Вкладка «1. Цены» — таблица дней / подпись / 50% / 100% + фикс трэп-бита (source of truth)',
          'Вкладка «2. Тексты» — подписи справки у клиента; суммы не вбивать руками — превью подставляет из таблицы',
          'Токены {{trap}} / {{p50_21}} спрятаны в «Дополнительно» — для старых шаблонов',
          'Сохранить — sticky-кнопка внизу',
        ],
      },
      {
        type: 'tip',
        text: 'Обычно цена при предоплате 50% ≥ полной 100%. Если наоборот — админка покажет предупреждение.',
      },
    ],
  },
  {
    ...
    title: 'Промо',
    group: 'Сайт',
    adminPath: '/admin/promo',
    keywords: ['sale', 'percent', 'fixed', 'scope', 'купон', 'одноразовый', 'username', 'скидка', 'промокод', 'акция'],
    blocks: [
      {
        type: 'p',
        text: 'Один раздел «Промо» с двумя вкладками: Акции (витринные скидки) и Промокоды (персональные одноразовые коды). Старые URL /admin/sales и /admin/promo-codes редиректят сюда.',
      },
      {
        type: 'h3',
        text: 'Акции',
      ...
      {
        type: 'p',
        text: 'Витринные кампании: scope all / beats / courses / services / ads; kind percent или amount; даты; enabled. На карточках — цена «было» (strikethrough). Скидки работают вместе с промокодами по правилам quote на сервере.',
      },
      {
        type: 'p',
      ...
      },
      {
        ...
        items: [
          ...
          'Промо → Промокоды → создать код, скопируйте, отправьте человеку.',
  ...
  },
  {
    ...
    keywords: ['oauth', 'totp', 'captcha', 'visibility', 'basic', 'maintenance'],
    blocks: [
      {
        type: 'p',
        text: 'Маршрут называется oauth-settings исторически: видимость разделов, безопасность и OAuth. Прайс услуг и ставка рекламы сюда не входят — смотри сайдбар «Сайт».',
      },
      {
        type: 'ul',
        items: [
          'courses_visibility — all / admins_only / hidden',
          'ads_orders_enabled — витрина /order/ads (ставка ₽/день — в Баннерах)',
      ...
      {
        type: 'tip',
        text: 'Ставку и очередь заявок удобнее править в Баннеры → Заявки. Скидку на рекламу — в Промо → Акции (scope ads).',
      },
      {
        type: 'warn',
    ...
    blocks: [
      {
        type: 'ul',
        items: [
          '«Знаю что нужно» — wizard; «?» на шагах Услуги/Итог открывает прайс; доп. связь — несколько каналов',
    ...
    blocks: [
      {
        ...
        items: [
          ...
          'Если акция — Промо → Акции или Баннер с датами',
    ...
    blocks: [
      {
        type: 'ol',
        items: [
          'Промо → Акции → scope + %/сумма + даты + enabled',
    ...
    blocks: [
      {
        ...
        items: [
          ...
          'Промо → Промокоды → создать → скопировать',
